...
2. **Frontend**:
   ...
   **npm version / lockfile:** CI installs with **npm 10** (Node 24). A locally
   installed npm 11 (Node 25+, which also dropped the bundled corepack) resolves
   `package-lock.json` differently and makes CI's `npm ci` fail. To keep the
   lockfile CI-compatible regardless of local npm:
   - `package.json` pins `"packageManager": "npm@10.9.2"`.
   - The `dev-setup` scripts arm `.githooks/pre-commit`, which **auto-relocks**
     the lockfile with the pinned npm (via `npx`) whenever frontend deps change.
   - Arm it manually with `git config core.hooksPath .githooks`; regenerate the
     lockfile by hand any time with `npm run relock` (from `lighthouse-front/`).
