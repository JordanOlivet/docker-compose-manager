const es = {
  app: {
    title: 'Lighthouse',
    composeManager: 'Gestión de Docker',
  },

  common: {
    loading: 'Cargando...',
    error: 'Error',
    success: 'Éxito',
    cancel: 'Cancelar',
    confirm: 'Confirmar',
    save: 'Guardar',
    delete: 'Eliminar',
    edit: 'Editar',
    create: 'Crear',
    refresh: 'Actualizar',
    forceRefresh: 'Actualizar forzadamente',
    back: 'Volver',
    close: 'Cerrar',
    search: 'Buscar',
    filter: 'Filtrar',
    clear: 'Limpiar',
    apply: 'Aplicar',
    next: 'Siguiente',
    previous: 'Anterior',
    submit: 'Enviar',
    retry: 'Reintentar',
    toggleSidebar: 'Alternar barra lateral',
    yes: 'Sí',
    no: 'No',
    all: 'Todos',
    none: 'Ninguno',
    selectLanguage: 'Seleccionar idioma',
    user: 'Usuario',
    group: 'Grupo',
    selectA: 'Seleccionar un',
    noAvailable: 'No hay {type}s disponibles',
    permissionsPreview: 'Vista previa de permisos',
    loadingPermissions: 'Cargando permisos...',
    noPermissionsFound: 'No se encontraron permisos',
    container: 'Contenedor',
    project: 'Proyecto',
    waitingForLogs: 'Esperando registros...',
    streamingPaused: 'Transmisión en pausa. Haga clic en "Reanudar" para continuar.',
    projectStatistics: 'Estadísticas del proyecto',
    noRunningServices: 'No hay servicios en ejecución para mostrar estadísticas.',
    environmentVariables: 'Variables de entorno',
    unknown: 'desconocido',
    selectFolder: 'Seleccionar carpeta',
    selectFile: 'Seleccionar archivo',
    select: 'Seleccionar',
    current: 'Actual',
    parentDirectory: '.. (Carpeta principal)',
    noSubdirectories: 'No se encontraron subcarpetas',
    noFiles: 'No se encontraron archivos',
    errorLoadingDirectories: 'Error al cargar la carpeta',
    enterOrSelectPath: 'Ingrese o seleccione una ruta',
    go: 'Ir',
    saving: 'Guardando',
    update: 'Actualizar',
    optional: 'opcional',
    performedAction: 'realizó {action}',
    onResource: 'en {type}',
    selectAll: 'Seleccionar todo',
    deselectAll: 'Deseleccionar todo',
    tryAdjustingSearch: 'Intenta ajustar tus criterios de búsqueda',
    days: 'días',
    crashLoop: {
      label: 'Crash loop',
      tooltip: 'Este contenedor está reiniciándose en bucle',
    },
  },

  navigation: {
    dashboard: 'Panel de control',
    containers: 'Contenedores',
    images: 'Imágenes',
    composeProjects: 'Proyectos Compose',
    composeFiles: 'Archivos Compose',
    logsViewer: 'Visor de registros',
    userManagement: 'Gestión de usuarios',
    userGroups: 'Grupos de usuarios',
    permissions: 'Permisos',
    appLogs: 'Registros de aplicación',
    settings: 'Configuración',
    categories: {
      overview: 'Vista general',
      docker: 'Docker',
      administration: 'Administración',
    },
  },

  permissions: {
    resourcePermissions: 'Permisos de recurso',
    copyFrom: 'Copiar de...',
    addPermission: 'Agregar permiso',
    noPermissionsAssigned: 'Aún no se han asignado permisos. Haga clic en "Agregar permiso" para comenzar.',
    resourceName: 'Nombre del recurso',
    selectResource: 'Seleccionar recurso',
    noResourcesAvailable: 'No hay recursos disponibles',
    permissions: 'Permisos',
    readOnly: 'Solo lectura',
    standard: 'Estándar',
  },
  
  auth: {
    login: 'Iniciar sesión',
    logout: 'Cerrar sesión',
    username: 'Nombre de usuario',
    password: 'Contraseña',
    currentPassword: 'Contraseña actual',
    currentPasswordPlaceholder: 'Ingrese la contraseña actual',
    newPassword: 'Nueva contraseña',
    newPasswordPlaceholder: 'Ingrese la nueva contraseña',
    confirmPassword: 'Confirmar contraseña',
    confirmPasswordPlaceholder: 'Confirme la nueva contraseña',
    changePassword: 'Cambiar contraseña',
    loginError: 'Nombre de usuario o contraseña inválidos',
    loginFailed: 'Error al iniciar sesión. Por favor, inténtelo de nuevo.',
    loggingIn: 'Iniciando sesión...',
    logoutSuccess: 'Sesión cerrada con éxito',
    passwordChanged: 'Contraseña cambiada con éxito',
    passwordMismatch: 'Las contraseñas no coinciden',
    passwordTooShort: 'La contraseña debe tener al menos 8 caracteres',
    passwordRequirements: 'Requisitos de la contraseña',
    passwordRuleTooShort: 'Debe tener al menos {min} caracteres',
    passwordRuleTooLong: 'No debe exceder {max} caracteres',
    passwordRuleUppercase: 'Debe contener al menos una letra mayúscula',
    passwordRuleLowercase: 'Debe contener al menos una letra minúscula',
    passwordRuleDigit: 'Debe contener al menos un dígito',
    passwordRuleSpecial: 'Debe contener al menos un carácter especial',
    passwordRuleWeak: 'No debe contener patrones débiles comunes (ej: password, 12345, admin)',
    changePasswordSubtitle: 'Por favor ingrese su contraseña actual y elija una nueva',
    defaultCredentials: 'Credenciales predeterminadas:',
    rememberMe: 'Recordarme',
  },
  
  dashboard: {
    title: 'Panel de control',
    subtitle: 'Monitoree su entorno Docker de un vistazo',
    stats: 'Estadísticas',
    health: 'Estado del sistema',
    healthy: 'Saludable',
    unhealthy: 'No saludable',
    database: 'Base de datos',
    docker: 'Docker',
    composePaths: 'Rutas Compose',
    recentActivity: 'Actividad reciente',
    noActivity: 'Sin actividad reciente',
    loadingActivity: 'Cargando actividad...',
    containers: 'Contenedores',
    totalContainers: 'Total de contenedores',
    composeProjects: 'Proyectos Compose',
    composeFiles: 'Archivos Compose',
    users: 'Usuarios',
    running: 'en ejecución',
    stopped: 'detenidos',
    active: 'activos',
    totalFilesTracked: 'Total de archivos rastreados',
    systemHealth: 'Estado del sistema',
  },
  
  images: {
    title: 'Imágenes',
    subtitle: 'Gestiona tus imágenes Docker',
    select: '',
    tags: 'Etiquetas',
    imageId: 'ID',
    size: 'Tamaño',
    age: 'Antigüedad',
    inUse: 'En uso',
    actions: 'Acciones',
    dangling: 'huérfana',
    self: 'self',
    unused: 'sin usar',
    usedByCount: 'usada por {n}',
    notSelectableHint: 'Las imágenes en uso o self no se pueden eliminar en masa',
    remove: 'Eliminar',
    selfProtected: 'Esta imagen ejecuta Lighthouse y no se puede eliminar',
    confirmRemove: 'Eliminar imagen',
    confirmRemoveWithName: '¿Eliminar la imagen {name}?',
    inUseHint: 'Imagen en uso — elimina primero el/los contenedor(es)',
    removeSuccess: 'Imagen eliminada',
    removeFailed: 'Error al eliminar la imagen',
    prune: 'Limpiar',
    pruneTitle: 'Limpiar imágenes sin usar',
    pruneDescription: 'Eliminar las imágenes que no usa ningún contenedor.',
    pruneDanglingOnly: 'Solo imágenes huérfanas',
    pruneDanglingOnlyHint: 'De lo contrario, eliminar todas las imágenes sin usar por ningún contenedor.',
    pruneSuccess: '{n} imagen(es) eliminada(s), {size} recuperado(s)',
    pruneFailed: 'Error al limpiar las imágenes',
    noImages: 'No se encontraron imágenes',
    noImagesMatch: 'Ninguna imagen coincide con tu búsqueda',
    deleteSelected: 'Eliminar selección ({n})',
    imagesSelected: '{n} seleccionada(s)',
    bulkDeleteTitle: 'Eliminar imágenes',
    bulkDeleteSubtitle: '{n} imagen(es) a eliminar',
    deleting: 'Eliminando',
    deletingProgress: 'Eliminando {current} / {total}',
    bulkDeleteSuccess: '{n} imagen(es) eliminada(s)',
    bulkDeletePartial: '{success} eliminada(s), {failed} con error',
    bulkDeleteFailed: 'Error al eliminar las imágenes',
  },

  containers: {
    id: 'ID',
    cpu: 'CPU %',
    ram: 'RAM',
    networkStats: 'Red (RX / TX)',
    diskStats: 'E/S de disco (Lectura / Escritura)',
    rx: 'RX',
    tx: 'TX',
    read: 'Lectura',
    write: 'Escritura',
    loadingDetails: 'Cargando detalles del contenedor...',
    readOnly: '(ro)',
    confirmRemoveWithName: '¿Eliminar el contenedor {name}?',
    confirmRemoveRunningWithName: 'El contenedor {name} está en ejecución. ¿Forzar eliminación?',
    title: 'Contenedores',
    subtitle: 'Gestione sus contenedores Docker',
    details: 'Detalles del contenedor',
    detailsSubtitle: 'Detalles y acciones del contenedor',
    backToContainers: 'Volver a contenedores',
    viewDetails: 'Ver detalles del contenedor',
    start: 'Iniciar',
    stop: 'Detener',
    restart: 'Reiniciar',
    remove: 'Eliminar',
    logs: 'Registros',
    stats: 'Estadísticas',
    name: 'Nombre',
    image: 'Imagen',
    state: 'Estado',
    status: 'Estado',
    created: 'Creado',
    ports: 'Puertos',
    ipAddress: 'Dirección IP',
    volumes: 'Volúmenes',
    networks: 'Redes',
    environment: 'Variables de entorno',
    labels: 'Etiquetas',
    actions: 'Acciones',
    technicalDetails: 'Detalles técnicos',
    liveResourceStats: 'Estadísticas de recursos en vivo',
    timeWindow: 'Ventana de tiempo',
    containerNotRunning: 'El contenedor no está en ejecución',
    noContainers: 'No se encontraron contenedores',
    noContainer: 'Sin contenedor',
    containerNotFound: 'Contenedor no encontrado',
    containerNotFoundMessage: 'No se pudo encontrar el contenedor solicitado',
    failedToLoad: 'Error al cargar los detalles del contenedor',
    failedToLoadMessage: 'Ocurrió un error inesperado',
    startSuccess: 'Contenedor iniciado con éxito',
    stopSuccess: 'Contenedor detenido con éxito',
    restartSuccess: 'Contenedor reiniciado con éxito',
    removeSuccess: 'Contenedor eliminado con éxito',
    startFailed: 'Error al iniciar el contenedor',
    stopFailed: 'Error al detener el contenedor',
    restartFailed: 'Error al reiniciar el contenedor',
    removeFailed: 'Error al eliminar el contenedor',
    confirmRemove: 'Eliminar contenedor',
    confirmRemoveRunning: 'El contenedor está en ejecución. ¿Forzar eliminación?',
    showAll: 'Mostrar todos',
    showRunning: 'Mostrar solo en ejecución',
    failedToStart: 'Error al iniciar el contenedor',
    failedToStop: 'Error al detener el contenedor',
    failedToRestart: 'Error al reiniciar el contenedor',
    failedToRemove: 'Error al eliminar el contenedor',
  },
  
  compose: {
    autoUpdateDisabled: 'Auto-actualización desactivada',
    autoUpdateDisabledHint: 'Este proyecto está excluido de las actualizaciones automáticas (x-auto-update: false en su archivo compose).',
    loadingDetails: 'Cargando detalles del proyecto...',
    confirmRemoveWithName: '¿Eliminar el proyecto compose {name}?',
    confirmRemoveRunningWithName: 'El proyecto compose {name} está en ejecución. ¿Forzar eliminación?',
    yaml: 'YAML',
    utf8: 'UTF-8',
    lines: 'líneas',
    characters: 'caracteres',
    title: 'Docker Compose',
    subtitle: 'Gestione sus proyectos Docker Compose',
    projects: 'Proyectos',
    projectName: 'Nombre / Ruta',
    files: 'Archivos',
    editor: 'Editor',
    details: 'Detalles del proyecto',
    projectDetails: 'Detalles del proyecto y servicios',
    backToProjects: 'Volver a proyectos',
    up: 'Iniciar',
    down: 'Detener',
    restart: 'Reiniciar',
    stop: 'Parar',
    recreate: 'Recrear',
    forceRecreate: 'Iniciar y forzar recreación',
    services: 'Servicios',
    noProjects: 'No se encontraron proyectos Compose',
    noProjectsMessage: 'Cree un archivo compose para comenzar',
    noFiles: 'No se encontraron archivos Compose',
    noFilesMessage: 'Agregue una ruta compose en configuración para comenzar',
    noServices: 'No hay servicios definidos en este proyecto',
    noComposeFile: 'No se encontró archivo compose para este proyecto. Las operaciones de archivo no están disponibles.',
    projectNotFound: 'Proyecto no encontrado',
    projectNotFoundMessage: 'No se pudo encontrar el proyecto solicitado',
    failedToLoad: 'Error al cargar proyectos compose',
    failedToLoadProject: 'Error al cargar detalles del proyecto',
    failedToLoadFiles: 'Error al cargar archivos compose',
    failedToLoadFile: 'Error al cargar archivo compose',
    failedToSave: 'Error al guardar',
    createFile: 'Crear archivo',
    editFile: 'Editar archivo',
    deleteFile: 'Eliminar archivo',
    deleteFileTitle: 'Eliminar archivo Compose',
    deleteFileMessage: '¿Está seguro de que desea eliminar este archivo?',
    filePath: 'Ruta del archivo',
    filePathPlaceholder: '/ruta/a/docker-compose.yml',
    fileContent: 'Contenido del archivo',
    directoryPath: 'Ruta del directorio',
    directoryPathPlaceholder: '/ruta/a/archivos/compose',
    unsavedChanges: 'Cambios no guardados',
    unsavedLeaveConfirm: 'Tiene cambios sin guardar. ¿Salir sin guardar?',
    fileSaved: 'Archivo guardado con éxito',
    invalidComposeFile: 'El archivo compose no es válido',
    composeTab: 'Archivo compose',
    envNew: 'nuevo',
    applyTitle: '¿Aplicar los cambios ahora?',
    applyDescription: 'El archivo se guardó pero los contenedores en ejecución aún usan la configuración anterior. ¿Redesplegar el proyecto ahora para aplicar los cambios?',
    applyNow: 'Redesplegar ahora',
    applyLater: 'Más tarde',
    conflictTitle: 'Archivo modificado en otro lugar',
    conflictDescription: 'Este archivo se modificó desde que lo abrió. ¿Recargar la última versión? Se perderán los cambios no guardados en esta pestaña.',
    conflictReload: 'Recargar la última versión',
    upSuccess: 'Proyecto iniciado con éxito',
    downSuccess: 'Proyecto detenido con éxito',
    restartSuccess: 'Proyecto reiniciado con éxito',
    stopSuccess: 'Proyecto detenido con éxito',
    startSuccess: 'Servicios del proyecto iniciados con éxito',
    confirmAction: '¿Está seguro?',
    confirmUpMessage: 'iniciar todos los servicios',
    confirmDownMessage: 'detener y eliminar todos los servicios',
    confirmRestartMessage: 'reiniciar todos los servicios',
    confirmStopMessage: 'detener todos los servicios',
    confirmRecreateMessage: 'recrear e iniciar todos los servicios (forzar recreación)',
  },
  
  users: {
    user: 'Usuario',
    admin: 'Administrador',
    title: 'Gestión de usuarios',
    subtitle: 'Gestione usuarios del sistema y sus permisos',
    groups: 'Grupos de usuarios',
    groupsSubtitle: 'Organice usuarios en grupos',
    permissions: 'Permisos',
    permissionsSubtitle: 'Gestione permisos de acceso a recursos',
    createUser: 'Crear usuario',
    editUser: 'Editar usuario',
    deleteUser: 'Eliminar usuario',
    username: 'Nombre de usuario',
    usernamePlaceholder: 'Ingrese el nombre de usuario',
    password: 'Contraseña',
    role: 'Rol',
    email: 'Correo electrónico',
    enabled: 'Habilitado',
    disabled: 'Deshabilitado',
    status: 'Estado',
    actions: 'Acciones',
    mustChangePassword: 'Debe cambiar la contraseña',
    createdAt: 'Creado el',
    lastLogin: 'Último inicio de sesión',
    noUsers: 'No se encontraron usuarios',
    userCreated: 'Usuario creado con éxito',
    userUpdated: 'Usuario actualizado con éxito',
    userDeleted: 'Usuario eliminado con éxito',
    userStatusUpdated: 'Estado del usuario actualizado',
    failedToCreate: 'Error al crear usuario',
    failedToUpdate: 'Error al actualizar usuario',
    failedToDelete: 'Error al eliminar usuario',
    general: 'General',
    groupName: 'Nombre del grupo',
    groupNamePlaceholder: 'Ingrese el nombre del grupo',
    description: 'Descripción',
    descriptionPlaceholder: 'Ingrese la descripción del grupo (opcional)',
    members: 'Miembros',
    noMembers: 'Sin miembros aún',
    leaveBlankToKeepCurrent: 'Dejar en blanco para mantener la contraseña actual',
  },
  
  settings: {
    title: 'Configuración',
    subtitle: 'Configure los ajustes de la aplicación',
    composePaths: 'Rutas Compose',
    composePathsTitle: 'Rutas de archivos Compose',
    addPath: 'Agregar ruta',
    addPathTitle: 'Agregar ruta Compose',
    removePath: 'Eliminar ruta',
    discoverFiles: 'Descubrir archivos',
    path: 'Ruta',
    status: 'Estado',
    access: 'Acceso',
    actions: 'Acciones',
    project: 'Proyecto',
    externalProjectsDetected: 'Proyectos detectados fuera de las rutas configuradas',
    pathLabel: 'Ruta',
    projectLabel: 'Proyecto',
    addThisPath: 'Añadir esta ruta',
    pathAdded: 'Ruta agregada con éxito',
    pathRemoved: 'Ruta eliminada con éxito',
    filesDiscovered: 'Archivos descubiertos con éxito',
    // Project update check settings
    projectUpdateCheck: 'Verificación de actualizaciones de proyectos',
    projectUpdateCheckDescription: 'Configure la frecuencia con la que el sistema verifica las actualizaciones de imágenes de contenedores en todos los proyectos compose.',
    checkInterval: 'Intervalo de verificación',
    intervalSaved: 'Intervalo de verificación guardado',
    // Tabs
    tabs: {
      general: 'General',
      appUpdate: 'Actualización de la app',
      projectUpdate: 'Actualizaciones de proyectos',
      notifications: 'Notificaciones',
      registry: 'Gestión de registros',
    },
    // Notifications (webhook de Discord)
    notifications: {
      discordTitle: 'Notificaciones de Discord',
      discordDescription:
        'Reciba un mensaje de Discord cuando se ejecute una actualización automática (autoactualización de la aplicación y de los proyectos compose). Las actualizaciones manuales no generan notificación.',
      enableLabel: 'Activar las notificaciones de Discord',
      webhookUrl: 'URL del webhook',
      webhookPlaceholder: 'https://discord.com/api/webhooks/...',
      webhookHelp:
        'Cree un webhook entrante en la configuración de su canal de Discord y pegue su URL aquí. El token se oculta tras guardar.',
      testButton: 'Enviar prueba',
      testing: 'Enviando…',
      testSuccess: 'Notificación de prueba enviada',
      testFailed: 'Error al enviar la notificación de prueba',
      saved: 'Configuración de notificaciones guardada',
      saveFailed: 'Error al guardar la configuración de notificaciones',
    },
    // General settings (log level)
    general: {
      title: 'Configuración general',
      logLevel: 'Nivel de registro de la aplicación',
      logLevelDescription:
        'Seleccione el nivel más detallado que se mostrará en los registros de la aplicación. Cualquier registro más detallado que el nivel seleccionado se suprime de inmediato, sin reiniciar la aplicación.',
      logLevelSaved: 'Nivel de registro actualizado',
      levels: {
        verbose: 'Verbose (Trace)',
        debug: 'Debug',
        information: 'Information',
        warning: 'Warning',
        error: 'Error',
        fatal: 'Fatal (Critical)',
      },
    },
    // Global compose env file
    composeEnv: {
      title: 'Archivo .env global de Compose',
      description:
        'Ruta a un archivo .env adicional aplicado en cada «up» y en cada actualización de imagen. Úselo cuando sus proyectos dependan de un .env compartido ubicado fuera del directorio del archivo compose: Docker no recuerda el --env-file usado al iniciar un proyecto por primera vez, por lo que se perdería cuando el gestor recrea los contenedores.',
      pathLabel: 'Ruta del archivo .env',
      pathPlaceholder: '/ruta/al/global.env',
      browse: 'Examinar',
      help: 'Déjelo vacío para desactivarlo. Un archivo inexistente se ignora (se registra) en lugar de hacer fallar la operación. Un .env local del proyecto, si existe, tiene prioridad sobre este archivo global.',
      saved: 'Archivo .env global guardado',
      saveFailed: 'Error al guardar el archivo .env global',
    },
    // Auto-update settings
    autoPrune: {
      title: 'Limpieza automática de imágenes',
      description: 'Elimina automáticamente las imágenes Docker sin usar según una programación para liberar espacio en disco. Las imágenes en uso por un contenedor (incluido Lighthouse) nunca se eliminan.',
      enableLabel: 'Activar la limpieza automática de imágenes',
      danglingOnlyLabel: 'Solo imágenes huérfanas',
      danglingOnlyHint: 'Desactivado: elimina toda imagen no usada por ningún contenedor.',
      cronLabel: 'Programación (expresión cron)',
    },
    autoUpdate: {
      composeTitle: 'Actualización automática',
      composeDescription: 'Actualiza automáticamente las imágenes y recrea los contenedores de los proyectos compose según una programación. Para excluir un proyecto, añada "x-auto-update: false" en la raíz de su archivo compose.',
      composeEnableLabel: 'Activar la actualización automática de proyectos',
      composeCronLabel: 'Programación (expresión cron)',
      appTitle: 'Actualización automática',
      appDescription: 'Verifica automáticamente GitHub según una programación. Si hay una nueva versión, la aplicación se actualiza y reinicia.',
      appEnableLabel: 'Activar la actualización automática de la aplicación',
      appCronLabel: 'Programación (expresión cron)',
      cronPlaceholder: 'ej. 0 2 * * * (cada día a las 02:00)',
      cronInvalid: 'Expresión cron inválida',
      nextRun: 'Próxima ejecución',
      countdownIn: 'en',
      countdownDay: 'd',
      countdownHour: 'h',
      countdownMinute: 'min',
      countdownSoon: 'pronto',
      cronUtcHint: 'La programación se evalúa en UTC (hora del servidor); la hora mostrada arriba está convertida a su hora local.',
      saved: 'Configuración guardada',
      saveFailed: 'Error al guardar la configuración',
    },
    // Registry management
    registry: {
      knownRegistries: 'Registros conocidos',
      customRegistries: 'Registros personalizados',
      configured: 'Configurado',
      notConfigured: 'No configurado',
      configure: 'Configurar',
      testConnection: 'Probar conexión',
      connectionSuccess: 'Conexión exitosa',
      connectionFailed: 'Error de conexión',
      connectionReachable: 'El registro es accesible',
      authenticatedConnection: 'Autenticado correctamente',
      loginTo: 'Iniciar sesión en',
      loginSuccess: 'Sesión iniciada correctamente',
      loginFailed: 'Error al iniciar sesión',
      logoutSuccess: 'Sesión cerrada correctamente',
      logoutFailed: 'Error al cerrar sesión',
      loggedInAs: 'Sesión iniciada como',
      addRegistry: 'Agregar registro',
      addCustomRegistry: 'Agregar registro personalizado',
      noCustomRegistries: 'No hay registros personalizados configurados',
      addCustomRegistryHint: 'Agregue un registro personalizado para autenticarse con registros privados',
      remove: 'Eliminar',
      registryUrl: 'URL del registro',
      urlRequired: 'La URL del registro es obligatoria',
      authType: 'Tipo de autenticación',
      usernamePassword: 'Usuario / Contraseña',
      accessToken: 'Token de acceso',
      tokenUsernamePlaceholder: 'Normalmente su usuario o déjelo vacío',
      tokenUsernameHint: 'Algunos registros requieren un usuario con autenticación por token (p. ej., GitHub usa su nombre de usuario)',
    },
  },

  appLogs: {
    title: 'Registros de aplicación',
    subtitle: 'Diagnostique el comportamiento de la aplicación en tiempo real',
    filters: 'Filtros',
    logLevel: 'Nivel de registro',
    logLevelSaved: 'Nivel de registro actualizado',
    clearLevels: 'Restablecer niveles',
    searchPlaceholder: 'Buscar en los mensajes...',
    categoryPlaceholder: 'Filtrar por categoría...',
    userPlaceholder: 'Filtrar por usuario...',
    linesCount: '{count} líneas',
    noLogs: 'Aún no hay registros.',
    autoScroll: 'Desplazamiento automático',
    pause: 'Pausar',
    resume: 'Reanudar',
    clear: 'Limpiar',
    download: 'Descargar',
    status: {
      idle: 'Inactivo',
      connecting: 'Conectando...',
      connected: 'En vivo',
      reconnecting: 'Reconectando...',
      disconnected: 'Desconectado',
    },
  },

  logs: {
    title: 'Visor de registros',
    subtitle: 'Vea registros de contenedores en tiempo real',
    streamingStopped: 'Transmisión detenida',
    selectContainer: 'Seleccione un contenedor para ver los registros',
    noLogs: 'No hay registros para mostrar',
    tail: 'Últimas líneas',
    waitingForLogs: 'Esperando registros...',
    clear: 'Limpiar',
    start: 'Iniciar',
    totalLogs: 'Total de registros',
    streaming: 'Transmitiendo...',
    containerTitle: 'Registros del contenedor',
    projectTitle: 'Registros del proyecto',
    searchPlaceholder: 'Buscar en los registros...',
    stderrOnly: 'Mostrar solo stderr',
    timestamps: 'Mostrar/ocultar marcas de tiempo',
    wrap: 'Ajuste de línea',
    pause: 'Pausar',
    resume: 'Reanudar',
    waiting: 'Esperando registros...',
    paused: 'Transmisión en pausa',
    connected: 'En vivo',
    reconnecting: 'Reconectando...',
    disconnected: 'Desconectado',
    goToBottom: 'Recientes',
  },
  
  errors: {
    generic: 'Ocurrió un error',
    failedToLoad: 'Error al cargar los datos',
    network: 'Error de red. Por favor, verifique su conexión.',
    unauthorized: 'No está autorizado para realizar esta acción.',
    notFound: 'Recurso no encontrado.',
    serverError: 'Error del servidor. Por favor, inténtelo de nuevo más tarde.',
    validationError: 'Error de validación. Por favor, verifique su entrada.',
    unknownError: 'Ocurrió un error inesperado.',
  },

  connection: {
    connected: 'Actualizaciones en tiempo real conectadas',
    connecting: 'Conectando...',
    reconnecting: 'Reconectando...',
    disconnected: 'Desconectado - actualizaciones pausadas',
    attempt: 'Intento',
    lastConnected: 'Última conexión',
  },

  update: {
    title: 'Actualización de la aplicación (solo para despliegues en Docker)',
    subtitle: 'Verificar actualizaciones y gestionar versiones',
    currentVersion: 'Versión actual',
    latestVersion: 'Última versión',
    checkForUpdates: 'Buscar actualizaciones',
    checkingForUpdates: 'Buscando actualizaciones...',
    updateAvailable: 'Actualización disponible',
    upToDate: 'Al día',
    updateNow: 'Actualizar ahora',
    updating: 'Actualizando...',
    lastChecked: 'Última verificación',
    never: 'Nunca',
    changelog: 'Registro de cambios',
    release: 'versión',
    releases: 'versiones',
    securityFixes: 'Correcciones de seguridad',
    breakingChanges: 'Cambios importantes',
    preRelease: 'Pre-lanzamiento',
    noReleaseNotes: 'No hay notas de la versión disponibles',
    viewOnGitHub: 'Ver en GitHub',
    showMore: 'Mostrar {count} versiones más',
    showLess: 'Mostrar menos',
    noChangelog: 'No hay registro de cambios disponible',
    confirmUpdate: 'Confirmar actualización',
    confirmUpdateMessage: 'La aplicación se reiniciará durante la actualización. Todos los usuarios serán desconectados temporalmente. ¿Desea continuar?',
    updateStarted: 'Actualización iniciada. La aplicación se reiniciará en breve.',
    updateFailed: 'Error en la actualización',
    checkFailed: 'Error al verificar actualizaciones',
    featureDisabled: 'La función de actualización automática está desactivada',
    maintenanceInProgress: 'Mantenimiento en progreso',
    applicationUpdating: 'La aplicación se está actualizando. Por favor espere...',
    reconnectingIn: 'Reconectando en {seconds}s...',
    attemptingReconnection: 'Intentando reconectar...',
    verifyingStability: 'Verificando estabilidad del sistema...',
    attemptCount: 'Intento {current} de {max}',
    reconnectionSucceeded: 'Reconexión exitosa',
    redirectingToLogin: 'Redirigiendo al inicio de sesión...',
    redirecting: 'Redirigiendo...',
    reconnectionFailed: 'Error de reconexión',
    maxAttemptsReached: 'Máximo de intentos alcanzado. Por favor actualice la página manualmente.',
    retryConnection: 'Reintentar conexión',
    // Compose project update keys
    checkUpdates: 'Verificar actualizaciones',
    updatesAvailable: 'actualizaciones disponibles',
    noImages: 'No se encontraron imágenes en este proyecto',
    localBuild: 'Build local',
    pinnedDigest: 'Digest fijado',
    disabled: 'Desactivado',
    local: 'Local',
    remote: 'Remoto',
    servicesSelected: 'servicios seleccionados',
    updateSelected: 'Actualizar selección',
    updateSuccess: '{count} servicios actualizados correctamente',
    restartFullProject: 'Reiniciar todo el proyecto',
    restartFullProjectHint: 'Reinicia todos los servicios del proyecto, no solo los actualizados',
    // Bulk update keys
    updateAll: 'Actualizar todo',
    updateAllProjects: 'Actualizar todos los proyectos',
    projectsHaveUpdates: 'proyectos tienen actualizaciones',
    updatingProject: 'Actualizando proyecto',
    projectsCompleted: 'proyectos completados',
    allProjectsUpToDate: 'Todos los proyectos están al día',
    servicesNeedUpdate: 'servicios necesitan actualización',
    projectsSelected: 'proyectos seleccionados',
    bulkUpdateSuccess: '{count} proyectos actualizados correctamente',
    bulkUpdatePartial: '{success} proyectos actualizados, {failed} fallidos',
    bulkUpdateFailed: 'Error al actualizar los proyectos',
    // Dev version keys
    devVersionNotice: 'Versión de desarrollo',
    newerImageAvailable: 'Una imagen más reciente está disponible para esta versión de desarrollo.',
    devUpdateInfo: 'Las versiones dev se actualizan comparando los digests de imágenes Docker.',
    // Progress tracking keys
    overallProgress: 'Progreso general',
    serviceProgress: 'Progreso por servicio',
    logs: 'Logs',
    pleaseWait: 'Por favor espere...',
    phase: {
      pull: 'Descargando imágenes',
      recreate: 'Recreando contenedores',
    },
    progress: {
      waiting: 'Esperando',
      pulling: 'Descargando',
      downloading: 'Descargando',
      extracting: 'Extrayendo',
      pulled: 'Descargado',
      recreating: 'Recreando',
      completed: 'Completado',
      error: 'Error',
    },
    // Container update keys
    containerUpdateSuccess: 'Contenedor actualizado con éxito',
    containerUpdateFailed: 'Error al actualizar el contenedor',
    standaloneWarning: 'Este contenedor no está gestionado por un archivo compose. La actualización recreará el contenedor con la misma configuración (puertos, volúmenes, entorno, redes).',
    checkContainerUpdates: 'Verificar actualizaciones',
    updateAllContainers: 'Actualizar todos los contenedores',
    containersHaveUpdates: 'contenedores tienen actualizaciones',
    allContainersUpToDate: 'Todos los contenedores están al día',
    containersSelected: 'contenedores seleccionados',
    bulkContainerUpdateSuccess: '{count} contenedores actualizados correctamente',
    bulkContainerUpdatePartial: '{success} contenedores actualizados, {failed} fallidos',
    bulkContainerUpdateFailed: 'Error al actualizar los contenedores',
  },
  actionLog: {
    title: 'Historial de acciones',
    noActions: 'Sin acciones',
    all: 'Todos',
    running: 'En curso',
    completed: 'Completado',
    failed: 'Fallido',
    cancelled: 'Cancelado',
    showLogs: 'Mostrar logs',
    hideLogs: 'Ocultar logs',
    lastActionFailed: 'Última acción fallida',
    viewDetails: 'Ver detalles',
    justNow: 'ahora mismo',
    minutesAgo: 'hace {count}min',
    hoursAgo: 'hace {count}h',
    daysAgo: 'hace {count}d',
    composeUp: 'Compose Up',
    composeDown: 'Compose Down',
    composeStart: 'Compose Start',
    composeStop: 'Compose Stop',
    composeRestart: 'Compose Restart',
    composeBuild: 'Compose Build',
    composePull: 'Compose Pull',
    containerStart: 'Iniciar contenedor',
    containerStop: 'Detener contenedor',
    containerRestart: 'Reiniciar contenedor',
    containerRemove: 'Eliminar contenedor',
    composeUpdate: 'Actualizar proyecto',
    containerUpdate: 'Actualizar contenedor',
    markSeen: 'Marcar como visto',
    markAllSeen: 'Marcar todo como visto',
  },
  user: {
    profile: 'Perfil',
    language: 'Idioma',
    darkTheme: 'Tema oscuro',
    preferences: 'Preferencias',
  },
  devTest: {
    title: 'Pruebas Docker reales',
    statusLoading: 'Cargando estado...',
    composeFiles: 'Archivos compose',
    createFiles: '1. Crear archivos',
    forceOutdated: '2. Forzar desactualizado',
    forceOutdatedHint: 'Cross-tagea imágenes para simular un digest obsoleto',
    restore: '3. Restaurar',
    restoreHint: 'Restaura las imágenes desde backups locales',
    teardown: '4. Eliminar archivos',
    running: 'En progreso...',
    lastOutput: 'Última salida',
    hint: 'Después de "Forzar desactualizado", ejecuta "Verificar MAJ" en la página principal para ver lighthouse-test-nginx y lighthouse-test-redis marcados como necesitando actualización.',
  },
}
export default es;
