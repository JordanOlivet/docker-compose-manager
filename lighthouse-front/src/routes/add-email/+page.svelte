<script lang="ts">
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			...
			// Keep the store in sync so guards don't bounce on a stale flag
			if (auth.user) updateUser({ ...auth.user, mustAddEmail: false });
			// If a password change is still pending, go there first; otherwise home
			goto(auth.user?.mustChangePassword ? '/change-password' : '/');
		} catch (error: any) {
			errorMessage =
